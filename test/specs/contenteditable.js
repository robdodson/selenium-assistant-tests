const {fixture, matchesKeyboard, matchesMouse} = require('../spec-helpers');

describe('<div contenteditable>', function() {
  beforeEach(async function() {
    fixture('contenteditable.html');
  });

  // This test seems to fail because FF won't focus a div with contenteditable
  // if it's the first element on the page. Weird.
  // https://jsbin.com/cawevo/edit?html,output
  it.skip('should apply .focus-ring on keyboard focus', async function() {
    await matchesKeyboard();
  });

  it.skip('should apply .focus-ring on mouse focus', async function() {
    await matchesMouse();
  });
});
